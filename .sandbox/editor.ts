...
import pkg1 from '@valajczech/alfons-bar-chart-plugin/editor';

const packages: { [pkgName: string]: PluginDefinition } = {
    '@valajczech/alfons-bar-chart-plugin': pkg1,
} as const;
...
const { Library, controls, commands } = Object.entries(packages).reduce(
    (acc, [pkgName, pkg]) => {
        ...
        const stripScope = (type) =>
            typeof type === 'string' ? type.replace(/^@[^/]+\//, '') : '';

        if (
            'schema' in definition &&
            !stripScope(pkgName).startsWith('flags-')
        ) {
            acc.Library[pkgName] = {
                ...definition,
